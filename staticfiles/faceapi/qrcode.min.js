...
// BeitDesk QR Code wrapper — compatible API with qrcode.js browser build
...
(function(QRCode) {
    ...
    QRCode.toCanvas = function(canvas, text, opts, callback) {
        try {
            var qr = makeQR(text, opts);
            var size = (opts && opts.width) ? opts.width : 256;
            ...
            var cellSize = Math.floor((size - margin * 2 * ((size / count) | 1)) / count);
            if (cellSize < 1) cellSize = 1;
            var total = count * cellSize + margin * 2;
            canvas.width = total;
            canvas.height = total;
    ...
    QRCode.toDataURL = function(text, opts, callback) {
