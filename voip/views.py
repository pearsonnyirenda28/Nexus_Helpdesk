...
def call_log(request):
    ...
    context = {
        ...
        'filters': {'status': status, 'direction': direction, 'search': search, 'date_from': date_from, 'date_to': date_to},
    ...
    if request.method == 'POST':
        ...
        if action == 'update_notes':
            ...
            ticket = Ticket.objects.create(
                ...
                description=f'Ticket created from VoIP call.\n\nCaller: {call.caller_number} ({call.caller_name})\nDuration: {call.talk_duration_display}\nDate: {call.initiated_at}\n\nNotes: {call.notes}',
            ...
            AuditLog.objects.create(
                ...
                ip_address=get_client_ip(request), notes=f'Created from VoIP call {call_uuid}',
            )
            messages.success(request, f'Ticket {ticket.ticket_id} created from this call.')
            return redirect('ticket_detail', ticket_id=ticket.ticket_id)

        elif action == 'end_call':
            if call.status in ['RINGING', 'ACTIVE', 'ON_HOLD']:
                call.status = VoIPCall.STATUS_COMPLETED
                if not call.answered_at:
                    call.status = VoIPCall.STATUS_MISSED
...
def log_call(request):
    ...
    if request.method == 'POST':
        ...
        call = VoIPCall.objects.create(
            ...
            destination_number=destination_number or 'HELPDESK',
            direction=direction,
            status=status,
            notes=notes,
        ...
        profile, created = CallerProfile.objects.get_or_create(phone_number=caller_number)
...
def caller_directory(request):
    ...
    page = request.GET.get('page', 1)
    context = {'callers': paginator.get_page(page), 'search': search}
    return render(request, 'voip/caller_directory.html', context)


def api_active_calls(request):
    """AJAX: live active calls data"""
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Unauthorized'}, status=401)
    now = timezone.now()
    calls = VoIPCall.objects.filter(status__in=['RINGING', 'ACTIVE', 'ON_HOLD']).values(
        'call_uuid', 'caller_number', 'caller_name', 'destination_number', 'status',
        'direction', 'initiated_at', 'answered_at', 'talk_duration_seconds',
    )
    call_list = []
    for c in calls:
        c['call_uuid'] = str(c['call_uuid'])
        if c['initiated_at']:
            elapsed = (now - c['initiated_at']).total_seconds()
